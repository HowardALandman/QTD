...
\title{An Elementary Approach to\\Quantum Time Dilation.}
\author{Howard A. Landman}
\date{v3.0\ \today} % delete this line to display the current date
...
\begin{abstract}
We present several arguments pointing towards a possible time dilation associated with the weak, strong, and electromagnetic potentials,
similar to the well-known gravitational time dilation of General Relativity.
This notion provides simple and direct links between GR and basic Quantum Mechanics.
Although a fairly complete and detailed theory with these exact properties was published by Apsel three decades ago,
it has been largely ignored and many objections to it have been made.
We show that most of these objections are obviously wrong, and that the remainder are less certain than they might appear.

Regardless of the truth or falsity of the theory, we show that it makes definite predictions and is readily and inexpensively testable,
and propose that such tests be performed.
\end{abstract}

\tableofcontents

%In General Relativity, clocks at different heights in a gravitational potential run at different rates, with higher ones running faster.
%In Quantum Mechanics, particles at different energy levels rotate their quantum phase at different rates, with the higher-energy one rotating faster.
%We propose here that these two phenomena are the same; that the phase oscillation of a particle can be viewed as its local clock,
%and that changes in phase frequency correspond to actual time dilations.
%We further demonstrate that the stationary time dilations predicted by QM and GR are identical.
%This elementary static theory of Quantum Time Dilation (QTD) leads us directly to predictions very similar to those made by an earlier dynamical theory by D. Apsel,
%which we discuss in historical context.
%Both theories predict that the lifetime of a muon will be affected by an electrostatic potential, and are therefore easily testable with relatively inexpensive equipment.

\section{The Unfinished Program of General Relativity}

To set the context, it will be helpful to review the history of General Relativity and attempts at unified theories.

Before relativity, Heaviside had attempted to unify Newtonian gravity with EM in his theory of Gravitomagnetism\cite{Heaviside1893}.
While GM had problems, and was rendered obsolescent by relativity theory,
it contained many interesting ideas and novel non-Newtonian predictions, such as measurable effects near rotating massive bodies.
In this sense, it can be viewed as an early glimpse of the kind of torsional GR effects measured by Gravity Probe B.
It is also worth noting that a revised and updated version of GM,
with fewer obvious problems, has been developed by Jefimenko\cite{Jefimenko1992};
this line of thought has not completely died out.

Between 1915-1917, when GR was first published in more-or-less final form, and the early 1930s, many people tried to find theories uniting GR with classical EM.
Analyzing this massive body of work is far beyond the scope of the present article;
even a book-length review\cite{Vizgin1994} was only able to cover some of the principal theories from this period.
One of the more famous attempts\cite{Weyl1922}, by H. Weyl, introduced the concept of gauge invariance,
which was crucial to many later developments.
Weyl wrote:
\begin{quote}
\ldots\ as the fact of the propagation of action and of the existence of rigid bodies leads us to found the affine on the {\em metrical} character of the world which lies a grade lower, it immediately suggests itself to us, not only to identify the co-efficients of the quadratic groundform $g_{ik}dx_{i}dx_{k}$ with the potentials of the gravitational field, but also to identify {\bf the co-efficients of the linear groundform $\phi_{i}dx_{i}$ with the electromagnetic potentials.}
\ldots\ We thus arrive at the inference:
{\bf The world is a $(3+1)$-dimensional metrical manifold; all physical field-phenomena are expressions of the metrics of the world.}\cite[p.283]{Weyl1922}
\end{quote}
None of these Unified Geometrized Theories (UGTs) was entirely successful; the problem was harder than people thought.
Both the triumphant development of Quantum Mechanics in 1925-27,
and the discovery of new elementary particles and the weak and strong forces a few years later,
made GR-EM unification seem increasingly irrelevant.
%The end of this period was marked by a talk given by Weyl (who himself had developed a UGT which didn't work).
%Weyl argued passionately that any successful unification would have to not be classical, but include QM.

For most working physicists from then on, GR was considered to be a theory of gravitation only, with little or no relevance for other phenomena.
Only a handful of researchers, including Einstein and Schr\"{o}dinger, felt otherwise.
In their view, the program of GR would not be complete until all forces had been geometrized:
%Schr\"{o}dinger's view of General Relativity anticipates gravitation and other forces all being treated as equally geometric.
%He writes
\begin{quote}
the conception Einstein put forward in 1915 embraced from the outset \ldots\ every kind of dynamical interaction, not just gravitation only.
\ldots\ the very foundation of the theory, viz. the basic principle of equivalence and a gravitational field,
clearly means that there is no room for any kind of 'force' to produce acceleration save gravitation,
which however is not to be regarded as a force but resides on the geometry of space-time.
Thus in fact, though not always in the wording, the mystic concept of force is wholly abandoned.
\ldots\ we are in patent need of field-laws for the matter-tensor
(e.g. for the electromagnetic field), laws that one would also like to conceive as purely geometrical restrictions on the structure of space-time.\cite{Schrodinger1950}
\end{quote}

Our first argument for electromagnetic (and weak, and strong) time dilation, while admittedly vague and qualitative, follows from this viewpoint.
We assume that such a unified geometrized theory can be formulated, with GR as a limiting case.
Such a theory would presumably treat each force somewhat similarly.
Since we know that there is a time dilation associated with the gravitational potential,
it would be reasonable to expect a similar time dilation to be associated with each of the other potentials.

The mainstream view is that in GR only gravity needs to be geometrized; other forces do not.
For example, S. Carroll writes
\begin{quote}
in GR, gravity is represented by the curvature of space-time, not by a force.
From the GR point of view, ``particles move along geodesics, until forces knock them off.''
Gravity doesn't count as a force.
If you consider the motion of particles under the influence of forces other than gravity, then they won’t move along geodesics\cite[p.14]{Carroll2001}.
\end{quote}
This is diametrically opposed to the Einstein-Schr\"odinger position.


\section{Time dilation and gravitation in GR}

It is easy to show that in general relativity, for slow-moving objects that are not close to a black hole,
almost all of the acceleration of gravity is due to time dilation.\cite{Hamza2015,TongGR,Nordebo2016,TerzicPHYS652}
For example, the Schwarzchild metric is:
\[ ds^2 = -(1-2GM/r)c^2dt^2 + (1 - 2GM/r)^{-1}dr^2 + r^2d\Omega^2 \]
and if we take the weak-field limit ($2GM/r$ is small)
and non-relativistic limit ($|dr/dt| << c$) we get
\[ ds^2 \approx -c^2dt^2 + dr^2 + r^2d\Omega^2 + 2\phi(r)c^2dt^2 \]
where $\phi(r) = GM/r$ is the Newtonian gravitational potential.
The first three terms are just the flat-space metric of special relativity,
so all of the curvature is in the time dimension;
yet the geodesics in this metric reproduce Newtonian gravitational dynamics.
So the traditional notion that gravity causes time dilation has it precisely backwards;
to a very good approximation, time dilation causes gravity.

Thus we have a concrete example of a mass-induced time dilation gradient presenting itself as a $1/r^2$ "force".
It therefore seems plausible that a similar charge-induced time dilation could present itself as the EM "force", which is also $1/r^2$.
Note, however, that any extension of GR to cover EM requires substantially different geometry.
Since positive and negative particles move in different directions under the influence of an electric field,
such a theory cannot have GR-style geodesics that are the same for all particles.


\section{Phase frequency as local clock: linking GR and QM}
The argument above are of course very unsatisfactory by themselves.
They make no quantitative predictions, and gives no hint of how to begin unifying GR with quantum theory.
Fortunately, elementary considerations afford us some further insights into this problem.
In GR, clocks deeper in a gravitational potential run slower.
In QM, particles deeper in an electric potential have lower phase frequency.
To make the analogy more precise is not difficult.

In GR the time dilation is normally given as a function of position within a gravitational potential, while in QM the phase shift is normally given as a function of the energy of a bound wave function.  To compare them, we need to put them into common terms.  We choose to do this by considering position in terms of energy, and phase shift in terms of time.
...
It is sufficient for our present purposes to consider only the simplest case of a uniform gravitational field with strength $g$.
The time dilation $T_d(h)$ between two observers at heights $h$ and $0$ is often given by the weak-field approximation\footnote{
For a derivation see e.g. \cite[section 48]{Lawden1982}.
...
For a general proof of the validity of synchronization in any stationary spacetime, see \cite[section 9.2]{Rindler2001}.)
}
The dimensionless ratio is the same in either case.
...
There is a kind of "event horizon" at $h = -\frac{c^2}{g}$ where time stops\footnote{
A uniform gravitational field is locally equivalent to an actual acceleration in flat spacetime.
For this situation, in the SR literature the event horizon is known as the {\em Rindler Horizon}
%and is often taken quite seriously
(see e.g. \cite{wikiRindler,Egan2006,Xiang2001,Culetu2006}).
%However this is fundamentally incorrect.
},
but this is purely an artifact of the approximation and has no basis in reality.
...
%}.
is the well-known unique exact solution for the uniform field\footnote{
It is e.g. equation 1.11 in \cite{Rindler2001}; see also chapter 9 of that work.
...
If we consider the mass to include the potential energy, so that a particle gets heavier as we raise it, and lighter as we lower it, 
then by an argument similar to that just given, we can conclude that the mass and energy depend exponentially on height
\[ m_h = m_0e^{gh/c^2} \]
\[ E_h = m_0c^2e^{gh/c^2} \]

A further confirmation of this comes observing that, if we have 3 observers at heights $a$, $b$, and $c$, the time dilations among them must be logically consistent.  By definition,
\[ T_d(c-b) \equiv t_c/t_b \]
\[ T_d(c-a) \equiv t_c/t_a \]
\[ T_d(b-a) \equiv t_b/t_a \]
So that the relation 
\[ T_d(c-a) = T_d(c-b) \cdot T_d(b-a) \]
must hold for any exact solution.
The exponential formula satisfies this, but the linear one does not.
...
Standing wave solutions to the Schr\"{o}dinger equation with energy $E$ oscillate phase as $e^{-iEt/\hbar}$.
%Absolute phase appears impossible to measure and may in fact have no physical meaning whatsoever; however, relative phase can be easily observed through various interference experiments.
For different energy levels of identical particles, higher energy will cause the wave function to rotate phase more rapidly.
Labeling the energy levels $h$ and $0$ as before and defining $\Delta E \equiv E_h - E_0$, we get a relative phase shift
...
%(More recent detailed particle models such as \cite{Hestenes2008} are also consistent with this view.)
...
We call this viewpoint Quantum Time Dilation (QTD).
%Indeed, any other interpretation seems problematic.
%On the one hand, it is quite obvious that any time dilation must affect all vibrational frequencies in direct proportion to the dilation.
%On the other, phase rotation seems to be the only deterministic clock-like attribute that elementary particles possess.

There is, of course, one other (non-deterministic, statistical) property of some elementary particles that could be used to measure their rate of time flow,
and that is their rate of decay.
Since the equations above indicate that any EM time dilation will be inversely proportional to the particle's rest mass,
it should be easiest to detect this in the lightest possible unstable particle, namely, the muon.
It is rather surprising that such elementary considerations as those given above can already make predictions about muon lifetimes
under a unified theory whose mathematical form we do not yet know in detail.
However, it seems unavoidable that any such theory must predict that the muon, with a mass of 105.7 MeV/c, will have its lifetime altered by about 1\%
in an electrostatic potential of 1.057 MV.

Of course, one would still like to have a detailed theory.  Which brings us to the curious case of David Apsel.
...
in late 1925 \cite{Schrodinger1925,Mehra2000}, when he gave the electron's frequencies in a hydrogen atom as
\[ \nu_n =mc^2/h - R/n^2 \]
except that Schr\"{o}dinger did not interpret this as a time dilation.
...
Thus the results of the previous section {\em could} have been derived as early as 1926.
%Why they weren't seems rather mysterious.

A similar time dilation was proposed by Apsel in 1978-9 \cite{Apsel1978,Apsel1979}.  His derivation starts from the Aharonov-Bohm effect, assuming that the variational principle
\[
\delta\int_A^Bd\tau = 0
\]
of relativity applies to both gravitational and electromagnetic fields.
He concluded, as we do, that ``the physical time associated with the trajectory of a classical particle is related to the beats of the quasi-classical quantum mechanical wave function associated with the particle''.
This seems to be exactly the sort of theory anticipated by Schr\"{o}dinger,
but despite experimental support\cite{Apsel1981}, it appears to have been largely ignored for three decades.
Only a handful of other papers \cite{Rodrigues1983,Ryff1985, Beil1987} refer to it.
...
He notes ``that not only does the electromagnetic energy tensor part \ldots\ appear in the curvature, but so does the matter term. Thus, one can say that everything in this theory is curvature.''
...
The consequences for particle lifetimes were not lost on Apsel;
he first proposed testing muon lifetimes in an electrostatic potential in his 1979 paper\cite{Apsel1979}.

\subsection{Related theories}

Time dilation due to very strong external fields has been considered several times,
for example by van Holten.
In a 1991 paper\cite{vanHolten1991} he derives the formula
\[ dt = d\tau\frac{E-q\phi}{M} \]
where $dt$ is the laboratory time and $d\tau$ is the proper time.
But $E$ and $M$ in this formula must be the same,
because in the absence of a potential ($\phi=0$) when laboratory time {\em is} proper time ($dt = d\tau$),
the formula gives
\[ \frac{dt}{d\tau} = 1 = \frac{E}{M} \]
In our notation his equation thus reduces to
\[ T+\Delta T = T\frac{E+\Delta E}{E} \]
which is equivalent to what we derived in section 1.
Two years later\cite{vanHolten1993} he gives the example of a muon spin down in an intense magnetic field
and predicts a change $\delta\tau$ in the muon lifetime $\tau_\mu$ given by
\[ \frac{\delta\tau}{\tau_\mu} = -\frac{\vec{\mu}\cdot\vec{B}}{mc^2} = 0.28\times 10^{-14} \times B \]
requiring a field strength on the order of 5 GT (producing an energy change of $|\vec{\mu}\cdot\vec{B}| \ge 1$ keV)
to see a significant variation in lifetime\cite{vanHolten1993}.
His calculations for the muon appear to give identical results to what we would predict for the same energy shift,
since his equation can be rewritten in our notation as
\[ \frac{\Delta T}{T} = \frac{\Delta E}{E} \]
which is also the same as ours from section 1.
However, Van Holten's theory differs notably from Apsel's in that it 
% is EM gauge invariant, and thus 
predicts effects from fields but not from potentials.
Essentially, he considers the effect to be a form of spin-orbit coupling.
Despite commenting that
``It is quite clear from this formula,
that any quantity which contributes to the energy $E$ in an observable way,
also contributes to the time dilation''\cite{vanHolten1993} (with which we heartily agree),
he does not in either paper consider that an electric potential can have any time dilation effect, particularly in a field free region.
%Instead, he fixates entirely on the idea of field strength.
This has significant implications for testability, because the same degree of dilation produced by the 5 GT field
(which is many orders of magnitude beyond what can be generated in labs today) in his theory
could, in the present theory and Apsel's theory, also be achieved by changing the potential of the muon by a kV or so
(something that could be easily done by almost anyone).
%Thus van Holten seems to imply that the effect is not easily testable,
%when in fact testing it even with relatively simple equipment should be quite straightforward.
...
The question of whether there might be some kind of time dilation associated with an electric potential was raised online in 2004 \cite{Creator2004}, but the discussion there was vague and inconclusive, and showed no awareness of Apsel's results.

More recent online discussions instigated by J. Duda (e.g. \cite{Duda2010}) are quite aware of Apsel, and also of the Heaviside \& Jefimenko theories\cite{Heaviside1893,Jefimenko1992} mentioned earlier, but appear so far to also be inconclusive.
Duda, like van Holten, seems to think that strong fields (and not just potentials) are required, saying e.g. ``Such experiment would need extremely strong electromagnetic field - like very near particles or near pulsars''.\cite[8 Jan 2010]{Duda2010}

In 2012 P. Ogonowski began publishing a theory of "Dilation As Field".\cite{Ogonowski2012,Ogonowski2013}
It concludes "that gravitational field as well as electromagnetic field may be considered through one phenomena - time dilation."
...
\subsection{Dismissal out of hand}

Some physicists reject the theory as so ridiculous that it does not even require an explanation of why it is wrong.
As this is useless for advancing the debate,
%and impossible to rebut,
...
the Aharonov-Bohm effect suffices as a counterexample.
...
%This is rather fatuous; it says that we can retain the idea that only fields (and not potentials) matter,
%as long as we are willing to allow the fields to act non-locally, something that in most other contexts would be pure heresy!

In any case, the NEGI idea (that fields acting locally on particles can explain everything) is admitted to be false.

\subsection{CPT and C Invariance}
...
Thus, the CPT theorem does not contradict the QTD claim that particles and antiparticles will be time-dilated oppositely at a non-zero potential and that their lifetimes will differ there.
...
This does not mean that the S-matrix approach is wrong; it merely means that the S-matrix is capable of expressing a time dilation.
...
%In special relativity, the time dilation for a particle or observer moving at velocity $v$ is $T_d = \sqrt{1 - v^2/c^2}$.
%The higher the velocity, the higher the kinetic energy, but the {\em slower} the clock is perceived to run.
%Thus kinetic energy does not appear to have the same relationship to time that potential energy does; even the sign is reversed.
%A further difficulty, as noted by de Broglie \cite{deBroglie1925}, is that while all observers agree on stationary time dilation,
%the time dilation due to motion is reciprocal (I think your clock is slower than mine at the same time you think my clock is slower than yours).
...
%Does it vanish?
%What happens to its charge, spin, and other properties?
...
In the above theories, the energy of a charged particle is a linear function of potential, and the time flow is likewise linear and has many of the same problems as the weak-field gravitational approximation.
...
One could conceive that the ``real'' theory, as in the case of gravitational time dilation, might be exponential rather than linear.
That would solve all the problems of the previous paragraph;
however, it would also appear to require abandoning the linear relation of frequency and energy ($E=h\nu$).
...
For all charged particles, QTD predicts changes in lifetime at non-zero electric potential.
Obviously this only has meaning for unstable particles.
With stable particles, one appears to  be limited to indirect tests based on interference effects


\subsection{Electrons and Positrons}

Electrons and positrons, with the largest charge-to-mass ratio of any particle,
would experience the greatest time dilation under the present theory.
However, since they have infinite lifetimes,
this would only be detectable as a phase shift which is identical to that predicted by standard theory.
%are ideal for detecting mass alteration due to electric potential.

%\subsubsection{Penning trap mass spectroscopy}
...
%\subsubsection{Inexpensive measurement of electron e/m}
%A cruder and cheaper test would use standard educational demonstration equipment for measuring the charge/mass ratio of the electron (such as the Cenco WL0623B or PASCO SE-9638), enclosing the device and power supplies in a Faraday cage and running the experiment at various potentials.
%(Assuming a spherical cage with radius 1m, and given that the capacitance of a conducting sphere in pF is roughly equal to its radius in cm\cite{Swartz2003}, we expect a capacitance of about 100 pF, and thus a total charge on the sphere at 500 kV of about 50 micro-coulombs.)

%Let's work through the expected results in detail.  The PASCO device accelerates electrons through a potential $V$ of 100-400 V.  The resulting velocity $v$ (at say 250 V) is given by
%\[ -qV = eV = \frac{1}{2}mv^2 \]
%\[ v = \sqrt{2eV/m} = \sqrt{500\rm{eV} /  (511\rm{keV}/ c^2)} = 0.0313 c \]
%The SR correction term at this speed is
%\[ \gamma = \sqrt{1 - v^2/c^2} = \sqrt{1 - 9.78\cdot 10^{-4}} = \sqrt{0.99902} = 0.99951 \]
%so we may use Newtonian mechanics with an error of about 0.05\%.
%Considering that the experiment is only accurate to 5\%, this should be adequate.

%We assume that the effective rest mass of the electron (with charge $q = -e$)is shifted by the external electric potential $\phi$ as
%\[ m(\phi) = E = m_0 - \frac{e\phi}{c^2} \]
%which is easily derivable from the de Broglie relation.  Thus the velocity is a function of $\phi$
%\[ v = v(\phi) = \sqrt{\frac{2eV}{m(\phi)}} = \sqrt{\frac{2eV}{m_0 - \frac{e\phi}{c^2}}} \]
%as is the momentum
%\[ p = p(\phi) = m(\phi)v(\phi) = (m_0 - \frac{e\phi}{c^2})\sqrt{\frac{2eV}{m_0 - \frac{e\phi}{c^2}}} = \sqrt{2eV (m_0 - \frac{e\phi}{c^2}) }\]

%The magnetic field near the center of a pair of Helmholtz coils of radius $a$ with $N$ turns and current $I$ is
%\[ B = \frac{N\mu_0I}{(5/4)^{3/2}a} \]
%and the magnetic force on the electron is
%\[ \vec{F}_B = e \vec{v}\times\vec{B} \]
%and since $\vec{v}$ and $\vec{B}$ are perpendicular we get simply that
%\[ F_B = evB \]
%which must equal
%\[ F_c = \frac{mv^2}{r} \]
%in order to hold the electron in a circular orbit, so that the charge-to-mass ratio is given by
%\[ \frac{e}{m} = \frac{v}{Br} \]
%and the radius by
%\[ r = \frac{mv}{eB} = \frac{m(\phi)v(\phi)}{eB} = \frac{p(\phi)}{eB} = \frac{1}{eB} \sqrt{2eV (m_0 - \frac{e\phi}{c^2})} \]
%Thus we predict that, for a fixed $V$ and $B$, the radius $r$ will vary with potential as
%\[ r(\phi) = r(0)\sqrt{1 - \frac{e\phi}{m_0c^2}} \]
%It has not escaped our attention that this formula gives an imaginary radius for $\phi > m_0c^2/e \approx +511$ kV.
%Testing the actual physical behavior for this range of potentials is thus a primary goal of the experiment.
%It is possible that our formula is only an approximation to the ``real'' formula (whatever that may be) and
%is only valid when $\left|e\phi\right|$ is small relative to $m_0c^2$.  One possible candidate for such a formula might be constructed by noting that
%\[ e^x \approx 1 + x \]
%for small $x$, giving
%\[ r(\phi) \approx r(0)\sqrt{e^{- \frac{e\phi}{m_0c^2}}} = r(0)e^{- \frac{e\phi}{2m_0c^2}} \]
%which has the merit of having no discontinuities and real values everywhere.
%Physical justification of such a formula might be tricky, although it is worth noting that mass scaling with a constant charge/mass ratio,
%and hence charge scaling as mass does, would give exactly this result.
%it is also possible that special-relativistic considerations might avoid the ``event horizon'' at  $e\phi = m_0c^2$.


%\subsubsection{Electron Interference}
One indirect test would be to set up an electron interference experiment and run it at various potentials.
Near $\phi = +511$keV, $h\nu = E = m_0c^2 - e\phi$ predicts the electron phase frequency should go to zero, and its de Broglie wavelength go infinite; therefore the interference fringes should get wider as we approach that voltage and disappear when we hit it exactly.
If this fails to happen, then the linear version of the theory cannot be right.

This sort of experiment would be easy to do inside a large Van De Graaff generator, such as the one at Museum of Science in Boston,
and not much more difficult in a moderately large enthusiast VDGG of say 0.5 meter diameter.
The main difficulty would be getting the experiment to run off battery power and fit in the space allowed.
...
%we would expect a QTD effect to be noticeable at that level.

%Allowing the $p^-$ to drop into lower orbitals would increase the QTD effect,
%but might also increase the chance of annihilation with a nuclear $p^+$.
%Orbitals with with $\ell\ne 0$ (such as $2p$, $(n,\ell) = (2,1)$) have nodes through the nucleus and would reduce the probability of nuclear capture; however, they tend to decay very rapidly.
%Also, higher-energy photons would be required;
%how low $n$ can be made is probably limited by the practicalities of high-frequency lasers.

%Alternately, the experiment could be repeated for heavier noble gases such as neon and argon,
%which would give a stronger effect at the same $n$, but would also require more energetic photons.

\subsection{Muons}
Radioactive ions or unstable charged particles will decay slower (or faster) when time-dilated.
...
Surveys of methods of muon production can be found in \cite{Eaton1999} and \cite{Heffner1984}; see also section VI of \cite{Kuno2001}.
...
Beam sources may be continuous or pulsed.
Muon lifetime detectors adequate to measure time dilation effects can be simple and inexpensive enough to be used in an undergraduate physics lab \cite{Coan2006}.

%Pions decay into muons by e.g.
%\[ \pi^- \rightarrow \mu^- + \bar{\nu}_\mu \]
%In the rest mass frame of the pion, the starting energy is just the rest mass of the pion, and the starting momentum is zero, so by conservation we must have
%\[ \vec{p}_{\mu} = - \vec{p}_\nu \]
%\[ E_\pi = m_\pi c^2 = E_\mu + E_\nu \]
%\[ E_\mu = \sqrt{m_\mu^2 + p_\mu^2} = \sqrt{m_\mu^2 + p_\nu^2}  \]
...
In the following subsections we look at several ways of altering muon lifetimes.
Much of what is said would apply equally well to other unstable particles such as pions.

\subsubsection{Electrostatic Potential Effect On Muons}

As mentioned above, QTD predicts that muon lifetime should be altered by an electrostatic potential.
Since the muon mass-energy is 0.1134 AMU\cite{PDG2014} = 105.6 MeV,
a potential of 1.056 MV should alter the lifetime by 1\%.
Such a potential could be expected on a VDGG of about 70 cm diameter in air, which is within the reach of a serious hobbyist.
A 1\% effect is huge by HEP standards and should be easy to detect.
Apsel (1979)\cite{Apsel1979} first proposed this kind of experiment;
37 years later, it still has never been performed.

Deep but small EM potentials exist around atomic nuclei.
Thus, QTD (like Apsel\cite{Apsel1981}) predicts that decay times of bound $\mu^-$ in matter should be longer,
and the lifetime should increase with atomic number.
Such an effect does seem to be observable in light atoms, but is traditionally explained by other means,
including special-relativistic time dilation from the orbital kinetic energy\footnote{
99\% of bound muons end up in the 1S ground state, which has zero angular momentum.
For atomic orbitals with zero angular momentum, the probability current is everywhere zero.
Thus, explaining the change in decay time by SR time dilation requires assuming that the muon is ``moving" even though there is zero current,
which seems questionable.}
and a reduction in accessible states for the decay products.
Unfortunately, in heavy atoms, the muon orbital is so small that the muon spend much of its time {\em inside} the nucleus,
so nuclear capture dominates and it is difficult to measure the muon decay lifetime.
If a muon could be kept in a 2P orbital, or any other orbital which has a node through the nucleus and should thus have a greatly reduced rate of capture,
the decay rate might be more easily observable for higher N.
%(Muons and pions are not, however, suitable for PTMS mass measurement, because their very short lifetime is not compatible with the long setup time required.)

%\subsubsection{Muonic Atoms}
%Negative muons ($\mu^-$) in matter are rapidly  ($ < 10^{-9}$ S) decelerated and bound to atomic nuclei.\cite{Fermi1947}  In most cases ($\sim$99\%) the muon decays into the ground state $1S$ orbital.  Because the muon orbit is much smaller than any of the electron orbits, to first order we can ignore the effect of the electron charges and treat the interaction between the nucleus and the muon as a hydrogen-like atom.  The energy level is then given by the formula
...
%\end{table}
%The QTD dilation needs to be applied in addition to the usual special-relativistic dilation due to the motion of the muon in its orbital.
...
%In fact the crossover point, where normal decay and nuclear capture are about equally likely, occurs at $Z=10$.
...
%As noted earlier, excited state orbitals with $\ell \ne 0$ (such as $2p$) have nodes through the nucleus and thus very little overlap with it.
...
%In particular, capture of $\mu^-$ by gases, combined with appropriate laser excitation to keep the muons in states with non-zero angular momentum, might give enough data points to see the effect clearly without nuclear capture dominating it.
...
%Note on scintillation detectors: Bell\cite{Bell1955} believes that plating the surface of a scintillation detector with a reflective material like aluminum is counterproductive, since it replaces the very good critical-angle reflection of the material itself with the less-good reflectivity of the aluminum.
%In fact in some cases better results are had by roughening the surface of the material using fine grit.

\subsubsection{Aharonov-Bohm Effect On Muons}
The Aharonov-Bohm effect\cite{Aharonov1959} (actually first described by  Ehrenberg and Siday \cite{Ehrenberg1949}) is a phase shift induced by a magnetic vector potential in a region of space which has zero magnetic field.
We interpret this phase shift as an actual time dilation, and so predict that the experienced time (and hence decay rate in the laboratory frame) will be different on different sides of the solenoid in an Aharonov-Bohm setup even though the particles never encounter any field.  Although fairly weak fluxes are used in typical AB experiments (because only $3.9\cdot 10^{-7}$ gauss-cm$^2$ is required to rotate the electron phase by $2\pi$ \cite{Ehrenberg1949}), much stronger fields could be used to test the time dilation effect.  MRI machines with 10 tesla (= $10^5$ gauss) fields over areas greater than 100 cm$^2$ have been demonstrated, so total fluxes of $10^7$ gauss-cm$^2$ and up are quite feasible.
...
$\Delta t = 3.91\cdot10^{-23}\frac{3.14\cdot10^8}{3.9\cdot 10^{-7}} {\rm S} = 3.14\cdot10^{-8}{\rm S}$.
The flight time of the muon would then be about $10^{-7}$ S.
With a lifetime of 2.2 $\mu$S, and ignoring relativistic corrections, we would expect in the standard interpretation that a fraction $e^{-t/2.2\mu{\rm S}}$ of the muons on each path would remain undecayed
\[ e^{-0.1/2.2} =  e^{-0.04545} = 0.956 \]
so that about 4.4\% of the muons would decay on each path.
However, the time dilation predicted by the present theory would cause the fast-time path to experience a total time $(t+\Delta t)$ of $(0.1 + 0.03)\mu{\rm S}$ so that approximately 
\[ e^{-0.103/2.2} = e^{-0.04682} = 0.954 \]
would remain undecayed while on the slow-clock path
\[ e^{-0.097/2.2} = e^{-0.04409} = 0.957 \] would.
...
For a rather large core with $(r_o-r_i) = 1 {\rm m}$ and $l = 1 {\rm m}$ (weighing about 25 metric tons\footnote{This is only moderately large by HEP standards; the storage ring magnet at the BNL muon g-2 experiment weighs 700 tons.\cite{Hughes2001}}) we would get a time shift of 10 nS.
...
\subsection{Neutrons}
Neutrons, being uncharged, are not subject to time dilation by electrostatic potentials.
However they do have spin and hence a magnetic moment, and may be dilated by the Aharonov-Casher effect
in the same way that charged particles may be dilated by the Aharonov-Bohm effect.
Also, like everything, they are affected by gravitational time dilation.

%\subsubsection{The Colella-Overhauser-Werner Experiment}
%In 1975 Colella, Overhauser, and Werner published the first experiment to explicitly link gravitation and quantum mechanics.\cite{Colella1975}
%A beam of neutrons with 0.1445 nm wavelength was Bragg-split in a silicon crystal (with interatomic spacing also 0.14 nm),
%the two lower-intensity beams each further split in a second crystal,
%and two of those four beams recombined in a third crystal.
%Interference between these two beams produced two output beams
%whose relative intensity was dependent on the quantum phase of the neutrons.
%(The basic setup is very similar to a Mach-Zehnder interferometer, but using crystals instead of mirrors.)

%The interfering beams follow an approximately rhomboidal path.  The entire experiment can be rotated about the incoming beam axis; at angle 0 the rhombus is horizontal and gravity affects both paths equally, but with rotation, one path goes higher than the other and a phase shift is seen.

%While much of this phase shift can be interpreted as classical time delay (neutrons on the upper path lose momentum, travel more slowly, and arrive slightly later than neutrons on the lower path), there is also a small contribution from gravitational time dilation.
%Given a height difference of $\Delta z$ in a uniform gravitational field of $g = 1$G, and using the weak-field approximation
%\[ T_d(\Delta z) \equiv \frac{T(\Delta z)}{T(0)} \approx 1 + \frac{g\Delta z}{c^2} \]
%the time dilation is approximately
%\[ T_d(\Delta z) \approx 1 + \frac{9.81 \frac{\mathrm{m}}{\mathrm{S}^2} \cdot \Delta z}{\left(2.998\cdot10^8 \frac{\mathrm{m}}{\mathrm{S}}\right)^2} = 1 + \frac{9.81}{8.988\cdot 10^{16}}\frac{\Delta z}{\mathrm{m}} = 1 + 1.091\cdot 10^{-16}\frac{\Delta z}{\mathrm{m}} \]

%In the original COW paper, the relative phase shift $\beta$ is given as
%\[ \beta = q_{\mathrm{grav}}\sin\varphi \]
%where
%\[ q_{\mathrm{grav}} = 4\pi\lambda gh^{-2}M^2d(d+a\cos\theta)\tan\theta \]
%with $\lambda = 0.1445$ nm, $g = 1$G, $h =$ Planck's constant, $M =$ mass of the neutron, $\theta=22.1^\mathrm{o}$ is the Bragg angle, $a=0.2$ cm, and $d=3.5$ cm.
%The number of fringes expected from a $180^\mathrm{o}$ rotation is then
%\[ (\Delta N)_{180} = q_{\mathrm{grav}} / \pi \]

%Assuming the neutron momentum is nearly constant,
%to first order the time dilation is given by the average height difference,
%which is just the average distance between the paths times $\sin\varphi$ or
%\[ \Delta z \approx (d+a)\tan\theta\sin\varphi = 3.7 \mathrm{cm}\cdot 0.406 \cdot \sin\varphi = 1.502 \mathrm{cm}\cdot\sin\varphi  \]
%while the length of each path (ignoring deflection of the endpoint due to gravity) is
%\[ \ell = \frac{2(d+a)}{\cos\theta} = 7.987 \mathrm{cm}\]
%with the neutron momentum
%\[ p = \frac{h}{\lambda} = \frac{6.626\cdot10^{-34} {\rm m^2 kg / s}}{1.445\cdot10^{-10}{\rm m}} = 4.585\cdot 10^{-24} {\rm m}\cdot{\rm kg / s} \]
%their velocity (assuming non-relativistic speeds) about
%\[ v = \frac{p}{M} =  \frac{4.585\cdot 10^{-24} {\rm m}\cdot{\rm kg / s}}{ 1.675\cdot10^{-27} {\rm kg}} = 2.737\cdot10^3 {\rm m/s} \]
%and the total time of flight roughly
%\[ \Delta t = \frac{\ell}{v} = \frac{ 7.987\cdot 10^{-2} \mathrm{m} }{ 2.737\cdot 10^3 {\rm m/s}} = 2.918 \cdot 10^{-5} {\rm s} \]
%The neutron oscillation frequency is given by
%\[ \nu = \frac{Mc^2}{h} = \frac{ 1.675\cdot10^{-27} {\rm kg} \times (2.998\cdot 10^8 {\rm m/s})^2}{6.626\cdot10^{-34} {\rm m^2 kg / s}} = 2.272 \cdot 10^{23} {\rm Hz} \]
%and hence the total number of oscillations during the flight is
%\[ N \approx 2.272 \cdot 10^{23} {\rm s}^{-1} \times 2.918 \cdot 10^{-5} {\rm s} = 6.630 \cdot 10^{18} \]
%We thus expect the number of fringe shifts due purely to time dilation to be (to first order) for the actual average height difference of $\Delta z = \sin\varphi 1.502 {\rm cm}$,
%\[ (\Delta N)_{T_d} = 6.630 \cdot 10^{18} \times 1.091\cdot 10^{-16}\frac{\Delta z}{\mathrm{m}} = 723.3 \times 0.01502 \sin\varphi =  10.864 \sin\varphi \]
%Since the number of fringe shifts predicted from purely kinematic arguments was $\approx 19$, this should have been easily detectable.
%Obviously, something went wrong in either the experiment or in its analysis.


\subsection{Other Forces}
The above discussion has been entirely about gravity and electromagnetism.
However, other potentials should also cause similar time dilation.
...
This could be used on uncharged particles such as neutrons, while most of the above approaches require charged particles.
However, the effect for any realistic confinement may be too small to measure.

There are many other possibilities, including the strong and weak nuclear forces, but these few should suffice to demonstrate that quantum time dilation makes different predictions than the standard view of phase shift, and that the differences are accessible to experimental test.
...
%\section{Charge Conservation, Gauge Invariance, and Noether's Theorem}
%In several sections above we have hinted that charge conservation may not be sacrosanct.
%This is of course foolhardy and heretical, and requires justification.
%In this section we attempt to provide a sketch of how charge conservation might not hold.
%
%Charge conservation is often viewed as a result of Noether's Theorem and the gauge invariance of classical EM.
%We are particularly interested in the simple case of changing the absolute electric potential $\phi$ by adding a constant to it everywhere, without changing the magnetic vector potential $A$.  That is, we want
%\[ \phi' = \phi + C\]
%\[ A' = A \]
%The gauge transformation that leaves electromagnetic fields unaltered is
%\[ \phi' = \phi - \frac{\partial\chi}{\partial t} \]
%\[ A' = A + \nabla\chi \]
%where $\chi$ is any scalar potential.
%Equating the two gives
%\[ \frac{\partial\chi}{\partial t} = -C \]
%\[ \nabla\chi = 0 \]
%We can certainly choose $\chi$ to be constant over space and satisfy this.  Defining
%\[ \chi_0 \equiv \chi(x=0,y=0,z=0,t=0) \]
%this gives
%\[ \chi(x,y,z,0) = \chi_0 \ \forall x,y,z \]
%or in other words, $\chi$ is a function of $t$ only.  Thus $\frac{\partial\chi}{\partial t} = \frac{d\chi}{dt}$ and
%\[ \frac{d\chi}{dt} = -C \]
%with solutions
%\[ \chi(t) = -Ct + D \]
%for arbitrary constant $D$.
%
%It is here that the standard interpretation goes slightly awry.
%In QM this corresponds to a phase shift
%\[ \psi' = e^{iq\chi/\hbar}\psi \]
%and since an absolute phase shift cannot be detected, this produces no physical change.
%However if we write $\psi$ in its time-dependent form
%\[ \psi = \psi(\vec{r})e^{-iEt/\hbar} \]
%we see that
%\[ \psi' = e^{iq\chi/\hbar}\psi(\vec{r})e^{-iEt/\hbar} = \psi(\vec{r})e^{iq\chi/\hbar-iEt/\hbar} = \psi(\vec{r})e^{i(-qCt + D-Et)/\hbar} = \psi(\vec{r})e^{-i((qC+E)t - D)/\hbar}\]
%
%``contrary to the conclusions of classical mechanics, there exist effects of potentials on charged particles, even in the region where all the fields (and therefore the forces on the particles) vanish.''\cite{Aharonov1959}
...
We interpret the well known quantum phase shift at different energy levels as a time-dilation.  The mathematics of this is essentially identical to that of gravitational time dilation in general relativity, indicating perhaps a deep and simple connection between QM and GR.  This interpretation is shown to have measurable consequences which have some support in prior experimental data, and further experiments are proposed that could test its validity more directly.

\begin{thebibliography}{10}

\bibitem{Heaviside1893}
 O. Heaviside,
``A gravitational and electromagnetic analogy (Part I)'',
{\em The Electrician} 31: 281-282 (14 July 1893).
``A gravitational and electromagnetic analogy (Part II)'',
{\em The Electrician} 31: 359 (1893).
A somewhat error-laden transcription can be found at
\url{http://www.univer.omsk.su/omsk/Sci/Heaviside/gravitation.htm}.
...
This book also contains a version of Heaviside's articles, rewritten in more modern notation.
