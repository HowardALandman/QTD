...
\date{v2.0\ \today} % delete this line to display the current date
...
He began in section 18 by using Special Relativity to show that clocks at different X-positions in a reference frame accelerated in the X-direction cannot run at the same rate;
...
$T_d = e^{gh/c^2}$ or, for a more general gravitational potential $\Phi$,
$T_d = e^{\Delta\Phi/c^2}$.
...
David Apsel first proposed this kind of experiment in 1979\cite{Apsel1979};
...
There classes of counterarguments to Apsel-style theories.
...
One could conceive that the ``real'' theory, as in the case of gravitational time dilation, might be non-linear.
%That would solve the problems of the previous paragraph.
The obvious candidate for the non-relativistic electrostatic case,
indeed the only candidate consistent with the way we treat gravitation, is 
$T_d = e^{q\Delta V/mc^2}$.
...
This solves the problems of the previous paragraph,
but whether it has other problems is beyond the scope of this paper.
