...
#
# This version allows for a second species of decaying particle.
# It does not yet adjust the error analysis for that.

sub do_line {
my($a,$n,$tau,$nu,$nf,$tauf) = @_;
print "dataset=$a, n=$n, tau=$tau, nu=$nu (nu is currently ignored), nf=$nf, tauf=$tauf\n";
...
# hypothesis in the same format as the summary data files.
	...
	$one_minus_pf = exp(-$b/$tauf);
	$p = 1 - $one_minus_p;
	$pf = 1 - $one_minus_pf;
	$decay = $p * $n;	# number expected to decay in first bin
	$decayf = $pf * $nf;	# number expected to decay in first bin
	$AFILE = "$a.hyp2";
	open(AFILE,">$AFILE");
	for ($i=0; $i<997; $i++)
	{
...
&do_line('ZZ', 36111.059211, 2059.437050, 2.230005, 107.878447, 28.500005);
