...
In General Relativity, clocks at different heights in a gravitational potential run at different rates, with the higher one running faster.  In Quantum Mechanics, particles at different energy levels rotate their quantum phase at different rates, with the higher-energy one rotating faster.  We propose here that these two phenomena are the same; that the phase oscillation of a particle can be viewed as its local clock, and that changes in phase frequency correspond to actual time dilations.  We further demonstrate that the stationary time dilations predicted by QM and GR are identical.  These considerations lead us directly to a theory which makes predictions very similar to those made by an earlier theory of Apsel, which we discuss in historical context.  Both theories predict that the lifetime of a muon will be affected by an electrostatic potential, and are therefore easily testable with relatively inexpensive equipment.
...
Time dilation due to strong external fields has been considered several times,
...
However, Van Holten's theory differs notably from Apsel's in that it is EM gauge invariant,
and thus predicts effects from fields but not from potentials.
...
More recent online discussions instigated by J. Duda (e.g. \cite{Duda2010}) are quite aware of Apsel, and also of O. Heaviside's 1893 theory of Gravitomagnetism\cite{Heaviside1893} and recent revivals and extensions of it by O. Jefimenko\cite{Jefimenko1992} and others, but appear so far to also be inconclusive.
Duda, like van Holten, seems to think that strong fields (and not just potentials) are required, saying e.g. "Such experiment would need extremely strong electromagnetic field - like very near particles or near pulsars".(\cite{Duda2010} 8 Jan 2010)
...
%(Care needs to be taken in the analysis, since the electron kinetic energy will not change as it gets lighter if it is accelerated through the same voltage.  Thus its velocity must differ as in the previous experiment.  This can be measured by using a pulsed electron source and measuring the time of flight.)

%\subsubsection{Implications of Negative Results}
...
%A combination of no difference in charge/mass ratio (or no difference in interference fringes) at sufficiently different voltages,
%and no difference in electron charge at sufficiently different voltages,
%would conclusively prove both QTD and the De Broglie relation $h\nu = mc^2$ false.
...
(or with the further assumption of EM-gauge invariance, which renders potential irrelevant).
A true CPT (or C) reflection must invert all charges in the universe, which necessarily inverts all electric potentials as well.
...
QTD is completely compatible with the notion of CPT (or C) invariance.
...
%{\em Rep. Prog. Phys.} 70 1995Ð2065 (2007)
...
Determination of the ProtonÕs Pseudoscalar Coupling $g_P$",
...
Reply to "Comment on ÔQuantum Phase Shift Caused by Spatial Confinement" by Murray Peshkin,
...
%{\em C. R. Acad. Sci. Paris}, t.2 SŽrie IV, 581-585 (2001)
